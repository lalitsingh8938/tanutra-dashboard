...
import React, { useState } from "react";
import { useNavigate } from "react-router-dom";
import imageCompression from "browser-image-compression"; // Importing the image compression library

function KYCForm() {
  const [companyCertificate, setCompanyCertificate] = useState(null);
  const [businessPAN, setBusinessPAN] = useState(null);
  const [gstCertificate, setGstCertificate] = useState(null);
  const [companyCertificateBase64, setCompanyCertificateBase64] = useState(null);
  const [businessPANBase64, setBusinessPANBase64] = useState(null);
  const [gstCertificateBase64, setGstCertificateBase64] = useState(null);
  const navigate = useNavigate();

  const handleFileChange = async (e, setFile, setBase64, fileName) => {
    const file = e.target.files[0];
    if (file) {
      // Compress the image
      const options = {
      ...
      try {
        ...
        reader.onloadend = () => {
          ...
          setFile(compressedFile); // Store compressed file (optional)

          // Alert to confirm Base64 conversion
          alert(`${fileName} has been converted to Base64 and saved successfully!`);

          // Save to localStorage for each file
          const kycData = {
  ...
  const handleSubmit = (e) => {
    ...
    const kycData = {
      ...
      businessPAN: businessPANBase64,
      gstCertificate: gstCertificateBase64,
    };

    // Save the Base64 data to localStorage
    try {
      localStorage.setItem("kyc_documents", JSON.stringify(kycData));
      alert("All KYC documents saved successfully!");
      navigate("/PickupAdd"); // Navigate to the next page
    } catch (error) {
      console.error("Error saving to localStorage:", error);
      alert("Error saving KYC documents. Please try again.");
    }
  };
            ...
            <form onSubmit={handleSubmit} className="mt-12 bg-transparent">
              ...
              <div className="flex flex-wrap flex-cols items-center p-5">
                ...
                <FileUploadField
                  ...
                  file={companyCertificate}
                  onFileChange={(e) =>
                    handleFileChange(e, setCompanyCertificate, setCompanyCertificateBase64, "companyCertificate")
                  }
                />
                ...
                <FileUploadField
                  ...
                  file={businessPAN}
                  onFileChange={(e) =>
                    handleFileChange(e, setBusinessPAN, setBusinessPANBase64, "businessPAN")
                  }
                />
                ...
                <FileUploadField
                  ...
                  file={gstCertificate}
                  onFileChange={(e) =>
                    handleFileChange(e, setGstCertificate, setGstCertificateBase64, "gstCertificate")
                  }
                />
              </div>
